...
@section('content')
    <div class="section-header">
        <div>
            ...
            <p class="mb-0" style="font-size:0.82rem;color:var(--text-secondary);">{{ __('ui.audio_desc') }}</p>
        </div>
        <button class="btn btn-accent" onclick="document.getElementById('uploadModal').style.display='flex'">
            <i class="bi bi-cloud-upload me-1"></i> {{ __('ui.upload_file') }}
        </button>
    </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-3">
            <div class="audio-stat"><div class="audio-stat-val">{{ $sounds->count() }}</div><div class="audio-stat-lbl">{{ __('ui.announcements_sounds') }}</div></div>
        </div>
        <div class="col-md-3">
        ...
        <div class="col-md-3">
            <div class="audio-stat"><div class="audio-stat-val">{{ $mohClasses->count() }}</div><div class="audio-stat-lbl">{{ __('ui.moh_classes') }}</div></div>
        </div>
        <div class="col-md-3">
            <div class="audio-stat">
                ...
                <div class="audio-stat-lbl">{{ __('ui.total_size') }}</div>
            </div>
        </div>
    </div>
    ...
    <div style="background:rgba(var(--accent-rgb), 0.08);border:1px solid rgba(var(--accent-rgb), 0.2);border-radius:10px;padding:0.75rem 1rem;margin-bottom:1rem;font-size:0.82rem;color:var(--text-secondary);">
        ...
        {!! __('ui.audio_convert_info') !!}
    </div>

    {{-- Sounds table --}}
    <div class="data-table mb-4">
        <div style="padding:0.75rem 1rem;border-bottom:1px solid var(--border);font-weight:700;font-size:0.85rem;">
            <i class="bi bi-volume-up me-1" style="color:var(--accent);"></i> {{ __('ui.announcements_sounds') }}
        </div>
            ...
            <tbody>
                @forelse($sounds as $s)
                    <tr>
                        ...
                        <td>
                            <button class="btn-icon me-1" title="{{ __('ui.listen') }}" onclick="playAudio({{ $s->id }}, '{{ addslashes($s->name) }}')">
                                <i class="bi bi-play-circle"></i>
                            </button>
                            <form action="{{ route('audio.destroy', $s) }}" method="POST" class="d-inline" onsubmit="return confirm('{{ __('ui.confirm_delete_audio', ['name' => $s->name]) }}')">
                                @csrf @method('DELETE')
    ...
    <div class="data-table">
        <div style="padding:0.75rem 1rem;border-bottom:1px solid var(--border);font-weight:700;font-size:0.85rem;">
            <i class="bi bi-music-note-beamed me-1" style="color:var(--accent);"></i> {{ __('ui.moh_files') }}
        </div>
        <table class="table mb-0">
            <thead>
                <tr>
                    <th>{{ __("ui.name") }}</th>
                    <th>{{ __('ui.moh_class_name') }}</th>
                    <th>{{ __("ui.original_file") }}</th>
                    <th>{{ __("ui.duration") }}</th>
                    <th>{{ __('ui.size') }}</th>
                    <th style="width:120px;">{{ __("ui.actions") }}</th>
            ...
            <tbody>
                @forelse($moh as $m)
                    <tr>
                        ...
                        <td>
                            <button class="btn-icon me-1" title="{{ __('ui.listen') }}" onclick="playAudio({{ $m->id }}, '{{ addslashes($m->name) }}')">
                                <i class="bi bi-play-circle"></i>
                            </button>
                            <form action="{{ route('audio.destroy', $m) }}" method="POST" class="d-inline" onsubmit="return confirm('{{ __('ui.confirm_delete_audio', ['name' => $m->name]) }}')">
                                @csrf @method('DELETE')
                                <button type="submit" class="btn-icon danger" title="{{ __('ui.delete') }}"><i class="bi bi-trash3"></i></button>
                            </form>
                        </td>
                ...
                @empty
                    <tr><td colspan="6" class="text-center py-3" style="color:var(--text-secondary);">{{ __('ui.no_moh_uploaded') }}</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <div id="uploadModal" class="audio-modal-overlay" style="display:none;">
        <div class="audio-modal-box">
            <div class="audio-modal-header">
                <h6 style="font-weight:700;margin:0;"><i class="bi bi-cloud-upload me-2" style="color:var(--accent);"></i> {{ __('ui.upload_audio') }}</h6>
            ...
            <form action="{{ route('audio.upload') }}" method="POST" enctype="multipart/form-data" style="padding:1.25rem;">
                ...
                <div class="row g-3">
                    <div class="col-12">
                        ...
                        <input type="text" name="name" class="form-control" placeholder="{{ app()->getLocale() === 'fr' ? 'ex: Message d\'accueil' : 'e.g. Welcome message' }}" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">{{ __('ui.type') }} *</label>
                        <select name="category" id="uploadCategory" class="form-select" onchange="toggleMohClass()">
                            <option value="sound">{{ __('ui.announcement_sound') }}</option>
                            <option value="moh">{{ __('ui.moh_type') }}</option>
                        </select>
                    </div>
                    <div class="col-md-6" id="mohClassGroup" style="display:none;">
                        <label class="form-label">{{ __('ui.moh_class_name') }}</label>
                        <input type="text" name="moh_class" class="form-control" value="default" placeholder="ex: default, premium">
                        <small style="color:var(--text-secondary);font-size:0.72rem;">{{ __('ui.moh_class_hint') }}</small>
                    </div>
                    <div class="col-12">
                        <label class="form-label">{{ __('ui.audio_file_label') }} *</label>
                        <input type="file" name="file" class="form-control" accept=".wav,.mp3,.ogg,.flac,.m4a,.aac" required>
                        <small style="color:var(--text-secondary);font-size:0.72rem;">{{ __('ui.audio_formats_hint') }}</small>
                    </div>
                </div>
                <div class="d-flex gap-2 mt-4 justify-content-end">
                    <button type="button" class="btn btn-outline-custom" onclick="document.getElementById('uploadModal').style.display='none'">{{ __('ui.cancel') }}</button>
                    <button type="submit" class="btn btn-accent"><i class="bi bi-cloud-upload me-1"></i> {{ __('ui.upload_convert') }}</button>
                </div>
            </form>
        </div>
    </div>
